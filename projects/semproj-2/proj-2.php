...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['edit'])) {
        ...
        $result = $con->query("SELECT * FROM project2 WHERE id='$selectedStudentId'");
    ...
    if (isset($_POST['save'])) {
        foreach ($_POST['students'] as $studentId => $details) {
            ...
            $updateQuery = "UPDATE project2 SET team_no='$team_no', project_title='$project_title', project_status='$project_status', publication_type='$publication_type', publisher_name='$publisher_name' WHERE id='$studentId'";
    ...
    if (isset($_POST['add'])) {
        ...
        $insertQuery = "INSERT INTO project2 (name, roll_no) VALUES ('$newStudentName', '$newStudentRollNo')";
...
<title>Project details for sem-2</title>
<style>
...
<nav>
  ...
  <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
    ...
    <div class="container">
        ...
        $result = $con->query("SELECT * FROM project2");
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                $file_exists = file_exists("/path/to/uploaded/files/{$row['id']}.extension"); // Update path and extension
                echo "<td>";
                if ($file_exists) {
                    echo "<button type='button' disabled>Uploaded</button>";
                } else {
                    echo "<a href='/phpupload/upload.html?id={$row['id']}'><button type='button'>Upload</button></a>";
                }
                echo "</td>";
