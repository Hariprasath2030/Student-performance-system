...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['save'])) {
        foreach ($_POST['students'] as $studentId => $details) {
            ...
            $updateQuery = "UPDATE iat3 SET Tamil='$tamil', ADS='$ADS', DBT='$DBT', Maths='$Maths', OOPS='$OOPS', MPMC='$MPMC',  WS='$WS'  WHERE id='$studentId'";
    ...
    /* Adjusted Main table display */
          body {
            font-family: Arial, sans-serif;
               ...
               background-image: radial-gradient(rgba(238, 235, 235, 0), rgb(48, 42, 42)), url(mmmm.jpg);
  ...
  <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
    ...
    <div class="container">
        ...
        if ($result->num_rows > 0) {
            ...
            echo "<table>";
            echo "<tr><th>Roll No</th><th>Name</th><th>Tamil</th><th>ADS</th><th>DBT</th><th>Maths</th><th>OOPS</th><th>MPMC</th><th>WS</th><th>Delete</th></tr>";
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                echo "<td><input type='number' name='students[{$row['id']}][ADS]' value='" . $row['ADS'] . "' class='input-field'></td>";
                echo "<td><input type='number' name='students[{$row['id']}][DBT]' value='" . $row['DBT'] . "' class='input-field'></td>";
                echo "<td><input type='number' name='students[{$row['id']}][Maths]' value='" . $row['Maths'] . "' class='input-field'></td>";
                echo "<td><input type='number' name='students[{$row['id']}][OOPS]' value='" . $row['OOPS'] . "' class='input-field'></td>";
                echo "<td><input type='number' name='students[{$row['id']}][MPMC]' value='" . $row['MPMC'] . "' class='input-field'></td>";
                echo "<td><input type='number' name='students[{$row['id']}][WS]' value='" . $row['WS'] . "' class='input-field'></td>";
