...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['edit'])) {
        ...
        $result = $con->query("SELECT * FROM iat3 WHERE id='$selectedStudentId'");
    ...
    if (isset($_POST['save'])) {
        foreach ($_POST['students'] as $studentId => $details) {
            ...
            $PDS = mysqli_real_escape_string($con, $details['PDS']);
            $Maths = mysqli_real_escape_string($con, $details['Maths']);
            $CO = mysqli_real_escape_string($con, $details['CO']);
            $WP = mysqli_real_escape_string($con, $details['WP']);
            $DPSD = mysqli_real_escape_string($con, $details['DPSD']);
            $Psycho = mysqli_real_escape_string($con, $details['Psycho']);


            // Update the student's marks
            $updateQuery = "UPDATE iat3 SET Tamil='$tamil', English='$english', PDS='$PDS', Maths='$Maths', CO='$CO', WP='$WP',  DPSD='$DPSD' ,  Psycho='$Psycho' WHERE id='$studentId'";
    ...
    if (isset($_POST['add'])) {
        ...
        $insertQuery = "INSERT INTO iat3 (name, roll_no) VALUES ('$newStudentName', '$newStudentRollNo')";
...
<nav>
  ...
  <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
    ...
    <div class="container">
        ...
        $result = $con->query("SELECT * FROM iat3");
        // Display the data in a table
        if ($result->num_rows > 0) {
            echo "<form action='" . htmlspecialchars($_SERVER["PHP_SELF"]) . "' method='post'>";
            echo "<table>";
            echo "<tr><th>Roll No</th><th>Name</th><th>Tamil</th><th>English</th><th>PDS</th><th>Maths</th><th>CO</th><th>WP</th><th>DPSD</th><th>Psychology</th><th>Delete</th></tr>";
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                echo "<td><input type='number' name='students[{$row['id']}][PDS]' value='" . $row['PDS'] . "' class='input-field'></td>";
                echo "<td><input type='number' name='students[{$row['id']}][Maths]' value='" . $row['Maths'] . "' class='input-field'></td>";
                echo "<td><input type='number' name='students[{$row['id']}][CO]' value='" . $row['CO'] . "' class='input-field'></td>";
                echo "<td><input type='number' name='students[{$row['id']}][WP]' value='" . $row['WP'] . "' class='input-field'></td>";
                echo "<td><input type='number' name='students[{$row['id']}][DPSD]' value='" . $row['DPSD'] . "' class='input-field'></td>";
                echo "<td><input type='number' name='students[{$row['id']}][Psycho]' value='" . $row['Psycho'] . "' class='input-field'></td>";
