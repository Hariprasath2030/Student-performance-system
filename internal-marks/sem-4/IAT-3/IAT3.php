...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['save'])) {
        foreach ($_POST['students'] as $studentId => $details) {
            ...
            $updateQuery = "UPDATE iat3 SET ADB='$ADB', AJP='$AJP', ES='$ES', Maths='$Maths', OS='$OS', BM='$BM',  DAA='$DAA'  WHERE id='$studentId'";
  ...
  <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
    ...
    <div class="container">
        ...
        if ($result->num_rows > 0) {
            ...
            echo "<table>";
            echo "<tr><th>Roll No</th><th>Name</th><th>ADB</th><th>AJP</th><th>ES</th><th>Maths</th><th>OS</th><th>BM</th><th>DAA</th><th>Delete</th></tr>";
            
            while ($row = $result->fetch_assoc()) {
                echo "<tr>";
                echo "<td>" . $row['roll_no'] . "</td>";
                echo "<td>" . $row['name'] . "</td>";
                echo "<td><input type='number' name='students[{$row['id']}][ADB]' value='" . $row['ADB'] . "' class='input-field'></td>";
                echo "<td><input type='number' name='students[{$row['id']}][AJP]' value='" . $row['AJP'] . "' class='input-field'></td>";
                echo "<td><input type='number' name='students[{$row['id']}][ES]' value='" . $row['ES'] . "' class='input-field'></td>";
                echo "<td><input type='number' name='students[{$row['id']}][Maths]' value='" . $row['Maths'] . "' class='input-field'></td>";
                echo "<td><input type='number' name='students[{$row['id']}][OS]' value='" . $row['OS'] . "' class='input-field'></td>";
                echo "<td><input type='number' name='students[{$row['id']}][BM]' value='" . $row['BM'] . "' class='input-field'></td>";
                echo "<td><input type='number' name='students[{$row['id']}][DAA]' value='" . $row['DAA'] . "' class='input-field'></td>";
