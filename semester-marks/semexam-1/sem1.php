...
// Loop through each subject to calculate GPA
foreach ($creditHours as $subject => $creditHours) {
    $totalCredit = 0;
    $totalGradePoints = 0;

    // Check if the subject exists in the $gradePoints array
    if (isset($gradePoints[$subject])) {
...
if ($result->num_rows > 0) {
    ...
    echo "<table>";
    echo "<tr><th>Roll No</th><th>Name</th><th>TAMIL</th><th>ENGLISH</th><th>CTPS</th><th>MATHS</th><th>ITES</th><th>ECONOMICS</th><th>CTPS-LAB</th><th>ITES-LAB</th><th>EE-LAB</th><th>CROP-LAB</th><th>EA-LAB</th><th>Total Marks</th><th>Delete</th></tr>";
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        foreach ($subjects as $subject) {
            echo "<td><input type='text' name='students[{$row['id']}][$subject]' value='" . $row[$subject] . "' class='input-field grade-input' data-student-id='{$row['id']}' data-subject='{$subject}'></td>";
        }
        ...
        if ($totalGradePoints > 0) {
          ...
          echo "<td>No grades available</td>";
      }
...
?>
    ...
    <script>
        // Add event listeners to input fields to calculate and display the sum
        document.addEventListener('DOMContentLoaded', function() {
            ...
            gradeInputs.forEach(function(input) {
                input.addEventListener('input', function() {
                    ...
                    var total = 0;
                    var inputsInRow = document.querySelectorAll('.grade-input[data-student-id="' + studentId + '"]');
                    inputsInRow.forEach(function(input) {
                        total += parseFloat(input.value) || 0;
                    });
                    var totalMarksInput = document.querySelector('.total-marks-input[data-student-id="' + studentId + '"]');
