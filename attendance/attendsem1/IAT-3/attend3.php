...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['save'])) {
        foreach ($_POST['students'] as $studentId => $details) {
            ...
            $economics = mysqli_real_escape_string($con, $details['Economics']);
             ...
             $totalAttendance = ($tamil + $english + $ctps + $maths + $ites + $economics) / 6;

            // Update the student's marks
            $updateQuery = "UPDATE att_3 SET Tamil='$tamil', English='$english', CTPS='$ctps', Maths='$maths', ITES='$ites', Economics='$economics', Attendance_Percentage='$totalAttendance' WHERE id='$studentId'";
    ...
    if (isset($_POST['add'])) {
        ...
        $insertQuery = "INSERT INTO att_3 (name, roll_no) VALUES ('$newStudentName', '$newStudentRollNo')";
...
<html lang="en">
<head>
<title>IT Attendance sem-1 IAT-3</title>
<style>
        nav {
        background: #1b1b1b;
        padding: 10px 0;
...
<nav>
      ...
      <ul>
        ...
        <li>
          ...
          <a href="/login/student/attendance/attendance.php">Back</a>
        </li>
      </ul>
    </nav>
    ...
    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post" onsubmit="return validateForm()">
        ...
        $selectQuery = "SELECT * FROM att_3";
        $result = mysqli_query($con, $selectQuery);
        // Display the data in a table
        if ($result->num_rows > 0) {
            echo "<form action='" . htmlspecialchars($_SERVER["PHP_SELF"]) . "' method='post'>";
            echo "<table>";
            echo "<tr><th>Roll No</th><th>Name</th><th>Tamil</th><th>English</th><th>CTPS</th><th>Maths</th><th>ITES</th><th>Economics</th><th>Total (%)</th><th>Delete</th></tr>";
            ...
            while ($row = $result->fetch_assoc()) {
              ...
              echo "<td><input type='number' name='students[{$row['id']}][English]' value='" . $row['English'] . "' class='input-field' id='english_" . $row['id'] . "' onchange='updateAttendance(" . $row['id'] . ")'></td>";
              echo "<td><input type='number' name='students[{$row['id']}][CTPS]' value='" . $row['CTPS'] . "' class='input-field' id='ctps_" . $row['id'] . "' onchange='updateAttendance(" . $row['id'] . ")'></td>";
              ...
              echo "<td><input type='number' name='students[{$row['id']}][ITES]' value='" . $row['ITES'] . "' class='input-field' id='ites_" . $row['id'] . "' onchange='updateAttendance(" . $row['id'] . ")'></td>";
              echo "<td><input type='number' name='students[{$row['id']}][Economics]' value='" . $row['Economics'] . "' class='input-field' id='economics_" . $row['id'] . "' onchange='updateAttendance(" . $row['id'] . ")'></td>";
           // Calculate total attendance percentage
           $totalAttendance = ($row['Tamil'] + $row['English'] + $row['CTPS'] + $row['Maths'] + $row['ITES'] + $row['Economics']) / 6;

           echo "<td id='attendance_percentage_" . $row['id'] . "'>" . $totalAttendance . "%</td>";
            echo "<td><a href='delete3.php?id=" . $row['id'] . "' onclick='return confirm(\"Are you sure you want to delete this student?\")'><button type='button'>Delete</button></a></td>";
              echo "</tr>";
